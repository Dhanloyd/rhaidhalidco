        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 lg:gap-12">
          ...
          <div>
            ...
            <ul className="space-y-2">
              {["Home", "About Us", "Players", "Activities", "Contact"].map((item) => (
                <li key={item}>
                  <Link to={item === "Home" ? "/" : `/${item.toLowerCase().replace(" ", "-").replace("about-us", "about")}`}
                    className="text-sm opacity-70 hover:opacity-100 transition-opacity duration-200">{item}</Link>
                </li>
              ))}
            </ul>
          </div>
          <div>
            <h4 className="font-heading text-sm text-primary-foreground mb-4 uppercase tracking-wider">Shop Categories</h4>
            <ul className="space-y-2">
          ...
          <div>
            ...
            <div className="flex gap-2 mb-4">
              ...
              <button className="bg-primary hover:bg-primary-light text-primary-foreground px-4 py-2 rounded-md text-sm font-medium transition-colors"><Mail size={16} /></button>
            </div>
            <div className="flex gap-3">
              {[Facebook, Instagram, Twitter, Youtube].map((Icon, i) => (
                <a key={i} href="#" className="p-2 rounded-full bg-primary-dark/40 hover:bg-primary/60 text-footer-foreground hover:text-primary-foreground transition-all duration-200 hover:scale-110"><Icon size={16} /></a>
              ))}
            </div>
          </div>
        </div>
