...
const isPants = (cat: string) => PANTS_CATEGORIES.some(c => cat?.toLowerCase().includes(c));

const SizeStockBreakdown = ({ product }: { product: any }) => {
  const isBottoms = isPants(product.category);
  const hasSizes  = (product.available_sizes?.length ?? 0) > 0;

  if (!isBottoms && !hasSizes) {
    return <span className="text-xs text-muted-foreground">—</span>;
  }

  if (isBottoms) {
    const waists  = product.available_waist_sizes?.length
      ? product.available_waist_sizes
      : ["28","29","30","31","32","33","34","36","38","40"];
    const lengths = product.available_lengths?.length
      ? product.available_lengths
      : ["28","29","30","31","32","33","34"];
    const totalCombos = waists.length * lengths.length;
    const qtyPerCombo = totalCombos > 0
      ? Math.floor(product.stock_quantity / totalCombos)
      : 0;

    return (
      <div className="flex flex-wrap gap-1 max-w-[300px]">
        {waists.flatMap((w: string) =>
          lengths.map((l: string) => {
            const qty   = qtyPerCombo;
            const isLow = qty > 0 && qty <= 2;
            const isOut = qty === 0;
            return (
              <div
                key={`${w}-${l}`}
                className={`inline-flex flex-col items-center px-1.5 py-0.5 rounded border text-[10px] min-w-[46px]
                  ${isOut
              ...
              >
                <span className="font-medium leading-none">W{w}/L{l}</span>
                <span className="font-semibold leading-tight">{qty}</span>
              </div>
            );
          })
        )}
  ...
  // Regular sizes (tops, shoes, etc.)
  const sizes = product.available_sizes as string[];
  const qtyPerSize = sizes.length > 0
    ? Math.floor(product.stock_quantity / sizes.length)
    : 0;

  return (
    <div className="flex flex-wrap gap-1 max-w-[200px]">
      {sizes.map((s: string) => {
        const qty   = qtyPerSize;
        const isLow = qty > 0 && qty <= 2;
        const isOut = qty === 0;
        return (
          <div
            key={s}
            className={`inline-flex flex-col items-center px-2 py-0.5 rounded border text-[10px] min-w-[28px]
              ${isOut
                ? "border-red-200 bg-red-50 text-red-600 dark:border-red-900 dark:bg-red-950 dark:text-red-400"
                : isLow
                ? "border-yellow-200 bg-yellow-50 text-yellow-700 dark:border-yellow-900 dark:bg-yellow-950 dark:text-yellow-400"
...
const InventoryPage = () => {
  ...
  return (
    <div className="space-y-6">
      <h1 className="font-heading text-2xl uppercase tracking-wider text-foreground text-white">
