...
export default function OrdersPage() {
  ...
  const confirmShipping = async () => {
    ...
    const { error } = await supabase.from("orders").update({
      ...
      courier_url: courierInfo?.url ? `${courierInfo.url}${trackingNumber}` : null,
    ...
    const updated = {
      ...
      tracking_number: trackingNumber, courier: courierInfo?.name,
      courier_url: courierInfo?.url ? `${courierInfo.url}${trackingNumber}` : null,
    };
        ...
        <div className="rounded-xl overflow-hidden">
          <Table>
            ...
            <TableBody>
              {filtered.length === 0 ? (
                <TableRow>
                  <TableCell colSpan={7} className="text-center py-12 text-slate-500">No orders found</TableCell>
                </TableRow>
              ) : filtered.map((o) => {
                ...
                return (
                  <TableRow key={o.id} className="border-white/5 hover:bg-white/5 transition-all">
                    ...
                    <TableCell>
                      <span className={`px-2 py-1 rounded-full text-xs font-medium capitalize ${STATUS_COLORS[o.status] || "bg-white/10 text-white"}`}>
                        {o.status}
                      </span>
                    </TableCell>
                    <TableCell className="text-xs">
                      {o.tracking_number ? (
                        <div>
                          ...
                          <p className="text-slate-500">{o.courier}</p>
                        </div>
                      ...
                      <div className="flex items-center gap-1">
                        {next && (
                          <Button size="sm" variant="ghost" disabled={updatingId === o.id}
                            ...
                            {next === "shipped" ? "Ship" : next.charAt(0).toUpperCase() + next.slice(1)}
                          </Button>
                        )}
          ...
          {viewOrder && (
            <div className="text-sm space-y-4">
              <div className="grid grid-cols-2 gap-2">
                <div><p className="text-xs text-slate-400">Order ID</p>
                  <p className="font-mono text-indigo-400">#{viewOrder.id?.slice(0, 8).toUpperCase()}</p></div>
                <div><p className="text-xs text-slate-400">Date</p>
                  <p>{new Date(viewOrder.created_at).toLocaleDateString()}</p></div>
                ...
                <div><p className="text-xs text-slate-400">Total</p>
                  <p className="font-bold text-emerald-400">₱{Number(viewOrder.total).toLocaleString()}</p></div>
                <div><p className="text-xs text-slate-400">Payment</p>
                  <p className="capitalize">{viewOrder.payment_method} · {viewOrder.payment_status}</p></div>
              </div>
                ...
                <div className="p-3 rounded-xl bg-indigo-500/10 border border-indigo-500/20">
                  ...
                  {viewOrder.courier_url && (
                    <a href={viewOrder.courier_url} target="_blank" rel="noopener noreferrer"
                      className="text-xs text-indigo-400 underline mt-1 inline-block">Track Package →</a>
                  )}
                </div>
              )}
                    ...
                    <div key={s} className="flex items-start gap-3">
                      <div className="flex flex-col items-center">
                        <div className={`w-3 h-3 rounded-full mt-0.5 shrink-0 ${
                          isCurrent ? "bg-indigo-400 ring-2 ring-indigo-400/30" :
              ...
              {viewOrder.shipping_address && (
                <div><p className="text-xs text-slate-400 mb-1">Shipping Address</p>
                  <p className="text-slate-300">{viewOrder.shipping_address}</p></div>
              )}
          ...
          <div className="space-y-4 text-sm">
            ...
            <div>
              ...
              <Input value={trackingNumber} onChange={(e) => setTrackingNumber(e.target.value)}
                placeholder="e.g. 1234567890" className="bg-white/5 border-white/10" />
            </div>
            <div className="flex gap-2">
              ...
              <Button variant="ghost" className="flex-1 border border-white/10"
                onClick={() => { setShippingModal(null); setTrackingNumber(""); }}>Cancel</Button>
            </div>
          </div>
        </DialogContent>
      </Dialog>
