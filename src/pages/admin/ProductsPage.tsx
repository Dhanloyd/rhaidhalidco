...
const emptyForm = { name: "", description: "", price: 0, category: "apparel", image_url: "", badge: "", in_stock: true };
      ...
      <div className="flex items-center justify-between">
        ...
        <Dialog open={dialogOpen} onOpenChange={(o) => { setDialogOpen(o); if (!o) { setForm(emptyForm); setEditId(null); } }}>
          ...
          <DialogContent className="max-w-lg">
            <DialogHeader>
              <DialogTitle className="font-heading uppercase tracking-wider">{editId ? "Edit Product" : "New Product"}</DialogTitle>
            </DialogHeader>
                ...
                <TableBody>
                  {products.map((p) => (
                    <TableRow key={p.id}>
                      ...
                      <TableCell>
                        {p.badge && <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${p.badge === "hot" ? "bg-orange-100 text-orange-700" : "bg-blue-100 text-blue-700"}`}>{p.badge}</span>}
                      </TableCell>
                      <TableCell>
