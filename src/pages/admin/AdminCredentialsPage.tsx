...
import {
  ...
  ShieldCheck, ShieldAlert, KeyRound, UserCog, ShieldOff,
} from "lucide-react";
    ...
    <Dialog open={mode === "add" || mode === "edit"} onOpenChange={onClose}>
      <DialogContent className="bg-[#111827] text-white border-white/10 max-w-md">
        <DialogTitle className="flex items-center gap-3">
          ...
          <div>
            <p className="text-sm font-semibold">{mode === "add" ? "Add New Admin Account" : "Edit Admin Account"}</p>
            <p className="text-[11px] text-slate-400 font-normal mt-0.5">
              {mode === "add" ? "Create credentials for admin panel access" : `Editing: ${account?.username}`}
            </p>
          </div>
        </DialogTitle>
        ...
        <div className="flex gap-2 pt-1">
          <Button className="flex-1 bg-indigo-600 hover:bg-indigo-500 text-white gap-1.5 text-sm"
            ...
            {mode === "add" ? <><Plus size={13} /> Create Account</> : <><CheckCircle size={13} /> Save Changes</>}
          </Button>
          <Button variant="ghost" className="flex-1 border border-white/10 text-sm" onClick={onClose}>Cancel</Button>
        </div>
      </DialogContent>
...
export default function AdminCredentialsPage() {
  // Guard is handled by <SuperAdminRoute> in App.tsx, but double-check here too
  ...
  const statCards = [
    { title: "Total Accounts", value: accounts.length, icon: UserCog,     iconBg: "bg-indigo-500/20",  iconColor: "text-indigo-400"  },
    ...
    <div className="min-h-screen p-6 space-y-6 relative"
      ...
      <div className="flex items-center justify-between relative z-10 opacity-0" style={{ animation: "fadeUp 0.5s ease forwards" }}>
        <div>
          ...
          <h1 className="text-2xl font-bold text-white tracking-tight">Admin Credentials</h1>
          <p className="text-sm text-slate-400 mt-0.5">Manage admin accounts and login access</p>
        </div>
        ...
        {statCards.map((s, i) => (
          <div key={s.title}
            ...
            <div className="flex items-start justify-between mb-4">
              ...
              <KeyRound size={13} className="text-slate-600 group-hover:text-slate-400 transition-colors" />
            </div>
      ...
      {/* Table */}
      <div className="rounded-2xl overflow-hidden opacity-0 relative z-10" style={{ ...cardStyle, animation: "fadeUp 0.5s ease 0.3s forwards" }}>
        {/* Filter bar */}
        <div className="p-4 flex flex-wrap items-center gap-3" style={{ borderBottom: "1px solid rgba(255,255,255,0.06)" }}>
          <div className="flex items-center gap-1 p-1 rounded-xl" style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.07)" }}>
            {roleTabs.map(tab => (
              <button key={tab.key} onClick={() => setRoleFilter(tab.key)}
                className={`px-3 py-1.5 rounded-lg text-xs font-medium capitalize transition-all flex items-center gap-1.5 ${roleFilter === tab.key ? "bg-indigo-500/30 text-indigo-300" : "text-slate-500 hover:text-slate-300"}`}>
                {tab.label}
          ...
          <span className="text-xs text-slate-500">{filtered.length} account{filtered.length !== 1 ? "s" : ""}</span>
        </div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr style={{ borderBottom: "1px solid rgba(255,255,255,0.06)" }}>
                {["Username", "Role", "Password", "Status", "Created", "Last Updated", "Actions"].map(h => (
              ...
              {filtered.length === 0 ? (
                <tr>
                  <td colSpan={7} className="text-center py-16 text-slate-500">
                    ...
                    {search || roleFilter !== "all" ? "No accounts match your filters" : "No admin accounts yet. Click 'Add Admin' to get started."}
                  </td>
                </tr>
              ) : (
                filtered.map((account, i) => {
                  ...
                  return (
                    <tr key={account.id}
                      ...
                      <td className="px-5 py-4">
                        <div className="flex items-center gap-2">
                          <span className="font-mono text-xs text-slate-400">{revealed ? account.password_hash : "••••••••••••"}</span>
                          <button onClick={() => toggleReveal(account.id)} className="text-slate-600 hover:text-slate-300 transition-colors">
      ...
      <div className="flex items-start gap-3 p-4 rounded-2xl relative z-10 opacity-0"
        ...
        <div>
          ...
          <p className="text-[11px] text-slate-500 leading-relaxed">
            Passwords are shown for demo purposes. In production, store only bcrypt hashes via a secure Supabase RPC — never plain text.
