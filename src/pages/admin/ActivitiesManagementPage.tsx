...
const ActivitiesManagementPage = () => {
  ...
  const handleSave = async () => {
    ...
    setForm(emptyForm); setEditId(null); setImageFile(null); setImagePreview(null);
    setDialogOpen(false); fetchActivities();
  };
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <Dialog open={dialogOpen} onOpenChange={(o) => {
          ...
          <DialogTrigger asChild>
            <Button className="gap-2"><Plus size={16} /> Add Activity</Button>
          </DialogTrigger>
          <DialogContent>
            <DialogHeader>
            ...
            <div className="space-y-4 mt-4">
              ...
              <div>
                ...
                {imagePreview && <img src={imagePreview} className="mt-2 w-full h-32 object-cover rounded" />}
              </div>
      ...
      <Card>
        <CardContent className="pt-6">
          {loading ? (
            <div className="flex justify-center py-12"><div className="animate-spin h-8 w-8 border-b-2 border-primary" /></div>
          ) : filtered.length === 0 ? (
            ...
            <Table>
              <TableHeader>
                <TableRow>
                  ...
                  <TableHead>Date</TableHead><TableHead>Location</TableHead><TableHead>Actions</TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
                {filtered.map((a) => (
                  <TableRow key={a.id}>
                    <TableCell>
                      {a.image_url ? (
                        <img src={a.image_url} className="w-12 h-12 rounded object-cover"
                          onError={(e) => { (e.target as HTMLImageElement).style.display = "none"; }} />
                      ) : "—"}
                    </TableCell>
                    <TableCell>{a.title}</TableCell>
                    <TableCell>{a.event_date ? new Date(a.event_date).toLocaleDateString() : "—"}</TableCell>
                    <TableCell>{a.location || "—"}</TableCell>
                    <TableCell>
                      <div className="flex gap-1">
                        <Button variant="ghost" size="icon" onClick={() => handleEdit(a)}><Pencil size={14} /></Button>
                        <Button variant="ghost" size="icon" onClick={() => handleDelete(a.id)}><Trash2 size={14} /></Button>
                      </div>
                    </TableCell>
                  </TableRow>
                ))}
