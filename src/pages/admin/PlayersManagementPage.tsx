...
const StatInput = ({ label, value, onChange }: { label:string; value:string; onChange:(v:string)=>void }) => (
  <div>
    ...
    <Input type="number" step="0.1" min="0" placeholder="0.0" value={value} onChange={(e)=>onChange(e.target.value)} className="text-center" />
  </div>
);
...
const PlayersManagementPage = () => {
  ...
  const fetchPlayers = async () => {
    const { data } = await supabase.from("player_profiles").select("*").eq("is_deleted",false).order("display_order");
    setPlayers(data||[]);
  };
  ...
  const setStat = (key:string, value:string) => setForm((f)=>({...f,stats:{...f.stats,[key]:value}}));

  const buildStatsPayload = () => {
    const s = form.stats;
    const obj: Record<string,number|null> = {};
    ([["ppg",s.ppg],["rpg",s.rpg],["apg",s.apg],["spg",s.spg],["bpg",s.bpg],["fgp",s.fgp],["tpp",s.tpp],["ftp",s.ftp]] as [string,string][])
      .forEach(([k,v])=>{ obj[k]=v!==""?parseFloat(v):null; });
    return obj;
  };

  const handleSave = async () => {
    if(!form.name){ toast.error("Name required"); return; }
    const payload = { name:form.name,position:form.position||null,jersey_number:form.jersey_number||null,bio:form.bio||null,image_url:form.image_url||null,display_order:form.display_order,stats:buildStatsPayload() };
  ...
  const handleEdit = (p:any) => {
    const s=p.stats||{};
    setForm({ name:p.name,position:p.position||"",jersey_number:p.jersey_number||0,bio:p.bio||"",image_url:p.image_url||"",display_order:p.display_order,stats:{ppg:s.ppg??"",rpg:s.rpg??"",apg:s.apg??"",spg:s.spg??"",bpg:s.bpg??"",fgp:s.fgp??"",tpp:s.tpp??"",ftp:s.ftp??""} });
    setEditId(p.id); setDialogOpen(true);
  };

  // ── Soft delete ──
  const handleDelete = async (id:string) => {
    const { error } = await supabase.from("player_profiles").update({ is_deleted:true }).eq("id",id);
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-4">
        ...
        <Dialog open={dialogOpen} onOpenChange={(o)=>{ setDialogOpen(o); if(!o){ setForm(emptyForm); setEditId(null); } }}>
          <DialogTrigger asChild>
            <Button className="gap-2 bg-primary text-primary-foreground font-heading uppercase tracking-wider"><Plus size={16} /> Add Player</Button>
          </DialogTrigger>
          <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
            ...
            <div className="space-y-5 mt-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div><label className="text-sm font-medium text-foreground block mb-1">Position</label><Input value={form.position} onChange={(e)=>setForm({...form,position:e.target.value})} placeholder="PG, SG, SF, PF, C" /></div>
              </div>
              <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                {form.image_url ? (
                  ...
                  <Button variant="outline" size="sm" onClick={()=>fileRef.current?.click()} disabled={uploading} className="gap-2"><Upload size={14} />{uploading?"Uploading...":"Upload Photo"}</Button>
                )}
                <input ref={fileRef} type="file" accept="image/*" className="hidden" onChange={(e)=>e.target.files?.[0]&&uploadImage(e.target.files[0])} />
              </div>
              <div>
                <div className="flex items-center gap-2 mb-3"><div className="h-px flex-1 bg-border" /><span className="text-xs font-heading uppercase tracking-widest text-muted-foreground px-2">Season Stats</span><div className="h-px flex-1 bg-border" /></div>
              ...
              <Button onClick={handleSave} className="w-full bg-primary text-primary-foreground font-heading uppercase tracking-wider">{editId?"Update Player":"Create Player"}</Button>
            </div>
          </DialogContent>
        </Dialog>
      </div>
      ...
      <Card><CardContent className="pt-6">
        {filtered.length===0 ? <div className="text-center py-12 text-muted-foreground">No players yet</div> : (
          <Table>
            <TableHeader><TableRow><TableHead>Photo</TableHead><TableHead>Name</TableHead><TableHead>Position</TableHead><TableHead>#</TableHead><TableHead>PPG</TableHead><TableHead>RPG</TableHead><TableHead>APG</TableHead><TableHead>SPG</TableHead><TableHead>BPG</TableHead><TableHead>FG%</TableHead><TableHead>3P%</TableHead><TableHead>FT%</TableHead><TableHead>Actions</TableHead></TableRow></TableHeader>
            <TableBody>
              {filtered.map((p)=>{ const s=p.stats||{}; return (
                <TableRow key={p.id}>
                  ...
                  <TableCell>{s.ppg??"—"}</TableCell><TableCell>{s.rpg??"—"}</TableCell><TableCell>{s.apg??"—"}</TableCell>
                  <TableCell>{s.spg??"—"}</TableCell><TableCell>{s.bpg??"—"}</TableCell>
                  <TableCell>{s.fgp!=null?`${s.fgp}%`:"—"}</TableCell><TableCell>{s.tpp!=null?`${s.tpp}%`:"—"}</TableCell><TableCell>{s.ftp!=null?`${s.ftp}%`:"—"}</TableCell>
