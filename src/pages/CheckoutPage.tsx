...
import { ShoppingCart, MapPin, CreditCard, CheckCircle, ChevronRight, ChevronLeft } from "lucide-react";
      ...
      <div className="container mx-auto max-w-4xl">
              ...
              <Card>
                ...
                <CardContent className="space-y-3">
                  {paymentMethods.map((pm) => (
                    <button key={pm.id} onClick={() => setPaymentMethod(pm.id)} className={`w-full flex items-center gap-4 p-4 rounded-xl border transition-all ${paymentMethod === pm.id ? "border-primary bg-primary/5" : "border-border hover:border-primary/30"}`}>
                      <span className="text-2xl">{pm.icon}</span>
                      <span className="font-medium text-foreground">{pm.label}</span>
                      {paymentMethod === pm.id && <CheckCircle size={18} className="ml-auto text-primary" />}
                    </button>
                  ))}
                  {paymentMethod === "gcash" && (
                    <div className="mt-4 p-4 bg-muted rounded-xl">
                      <p className="text-sm text-muted-foreground">You will receive GCash payment instructions after placing your order.</p>
                    </div>
                  )}
                  {paymentMethod === "card" && (
                    <div className="mt-4 p-4 bg-muted rounded-xl">
        ...
        {step === 3 && (
          <div className="text-center py-16">
            ...
            <p className="text-sm font-mono text-muted-foreground mb-6">Order ID: {orderId?.slice(0, 8)}</p>
        ...
        {step < 3 && (
          <div className="flex justify-between mt-8">
            ...
            {step < 2 ? (
              ...
              <Button onClick={handlePlaceOrder} disabled={processing} className="gap-2 bg-primary text-primary-foreground font-heading uppercase tracking-wider">
