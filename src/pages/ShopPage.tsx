    ...
    <div className="bg-card rounded-xl border border-border/50 overflow-hidden hover-lift group">
        ...
        {product.badge && (
          <span
            className={`absolute top-3 right-3 ${
              product.badge === "hot" ? "badge-hot" : "badge-featured"
            }`}
          >
      ...
      <div className="p-5">
        ...
        <div className="flex items-center justify-between">
          ...
          <Button
            size="sm"
            className="bg-primary text-primary-foreground hover:bg-primary-light gap-1.5 font-medium"
...
const ShopPage = () => {
  ...
  const renderIcon = (cat: ProductCategory, size = "w-5 h-5") => {
    const icon = categoryIcons[cat];

    if (typeof icon === "string" && icon.includes(".jpg")) {
      return <img src={icon} alt="" className={`${size} inline mr-1`} />;
    }
      ...
      <section className="gradient-navy section-padding pt-24 md:pt-32">
        <div className="container mx-auto text-center">
          ...
          <p className="text-primary-foreground/70 max-w-2xl mx-auto text-lg mb-8">
            Official RaidKhalid & Co. merchandise and products.
          </p>
          ...
          <div className="flex flex-wrap justify-center gap-3">
            <Button
              ...
              className={`font-heading uppercase tracking-wider ${
                activeCategory === "all"
                  ...
                  : "border-primary-foreground/50 bg-transparent hover:bg-primary-foreground/10"
              }`}
              style={
                activeCategory !== "all"
                  ? { color: "hsl(0 0% 100%)" }
            ...
            {allCategories.map((cat) => (
              ...
              >
                {renderIcon(cat)} {categoryLabels[cat]}
              </Button>
            ))}
          </div>
        </div>
        ...
        <>
          ...
          <section className="section-padding">
            <div className="container mx-auto">
              {activeCategory !== "all" && (
                <h2 className="font-heading text-2xl md:text-3xl uppercase tracking-wider text-foreground mb-8">
                  {renderIcon(activeCategory as ProductCategory, "w-6 h-6")}{" "}
                  {categoryLabels[activeCategory as ProductCategory]}
                </h2>
              )}
